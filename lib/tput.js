...
function npmPackagePathResolver(packageName) {
  const gbPath = cp.execSync(`npm ls ${packageName} -g --parseable`).toString()
  return gbPath.trim();
}
Tput.prototype.getRootPath = function() {
  const gbPath =  npmPackagePathResolver('gitboard-cli')
  const filePath = path.join(gbPath,'node_modules','gitboard-cli-blessed','usr') 
  if(fs.existsSync(filePath)){
    return filePath
  }
  return __dirname + '/../usr'
};
Tput.prototype._useXtermCap = function() {
  const filePath = path.join(this.getRootPath(),'xterm.termcap') 
  return this.injectTermcap(filePath);
};

Tput.prototype._useXtermInfo = function() {
  const filePath = path.join(this.getRootPath(),process.platform === 'win32' ? 'windows-ansi' : 'xterm') 
  return this.injectTerminfo(filePath);
};

Tput.prototype._useInternalInfo = function(name) {
  name = path.basename(name);
  const filePath = path.join(this.getRootPath(),name)
  return this.injectTerminfo(filePath);
};

Tput.prototype._useInternalCap = function(name) {
  name = path.basename(name);
  const filePath = path.join(this.getRootPath(),name+'.termcap')
  return this.injectTermcap(filePath);
};
